...
def plot_zplane(z, p, k):
    ...
    rect_box = Rectangle((-3, -3), 6, 6, color='yellowgreen', alpha=0.33)
    ...
    plt.text(0, +1, 'k=%f' % k)
    plt.text(0, -1, 'ROC for causal: green')
    plt.axis('square')
    # plt.axis([-2, 2, -2, 2])
    plt.xlabel(r'$\Re\{z\}$')
    plt.ylabel(r'$\Im\{z\}$')
...
def plot_splane(z, p, k):
    ...
    if np.imag(pp) == 0:
        ...
        plt.fill_between([pp, 2*xmax], [2*ymax, 2*ymax],
                         2*ymin, color='yellowgreen', alpha=0.33, label='ROC')
    else:
        plt.fill_between([-2*xmax, 2*xmax], [2*ymax, 2*ymax],
                         2*ymin, color='yellowgreen', alpha=0.33, label='ROC')
    plt.xlabel(r'$\Re\{s\}$')
    plt.ylabel(r'$\Im\{s\}$')
    plt.legend()
...
def plot_clti_analysis(z, p, k,):
    ...
    # sys_ba = signal.TransferFunction(sys)
    # b = sys_ba.num   # we needed coeff b,a for group_delay()
    # a = sys_ba.den

    [W, H] = signal.lti.freqresp(sys)
    F = W/(2*np.pi)
    gd = group_delay(z=z, p=p, w=W)[1]  # gd in samples using poles / zeros
