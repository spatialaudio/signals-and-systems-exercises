...
%
Eine zeitliche Verschiebung um $\frac{T_h}{2}$ führt zu einer Phasenänderung (der
Betrag bleibt erhalten) in der
Fouriertransformierten, und zwar um $\angle \e^{-\im\omega\frac{T_h}{2}} = -\omega\frac{T_h}{2}$.

Damit ist für das hier berechnete Beispiel der Betrag der Fouriertransformierten
$|X(\im \omega)| = A T_h \cdot |\mathrm{sinc}(\omega\frac{T_h}{2})|$ und die Phase
der Fouriertransformierten
$\angle X(\im\omega) = -\omega\frac{T_h}{2}$.
%
...
\end{itemize}
%
Aufgabe \ref{sec:8C3958BE4F} lieferte eine reelle
Lösung, wir mussten daher in \fig{fig:8C3958BE4F_0} nur den Realteil
plotten.
%
Das können wir nun hier nicht mehr machen: wenn wir nur den Realteil darstellen,
verlieren wir durch unterschlagenen Imaginärteil Informationen.
%
...
%
Die Phase ist eine Gerade über $\omega$ durch den Ursprung mit Anstieg $-T_h/2$,
also fallend.
%
...
X(\im \omega) = A T_h \cdot \mathrm{sinc}(\omega\frac{T_h}{2}) \cdot  \e^{+\im\omega\frac{T_h}{2}}.
\end{equation}
ist in \fig{fig:1CFE5FE3A1} dargestellt. Der Betrag $A T_h \cdot |\mathrm{sinc}(\omega\frac{T_h}{2})|$
...
Der wichtige neue Punkt ist:
Die Phase ist eine Geradengleichung über $\omega$ durch den Ursprung und mit Anstieg $+T_h/2$,
also diesmal steigend!
%
...
%
Die Mathematik\footnote{\url{https://dlmf.nist.gov/1.17} bzw. Buch
\cite[Kapitel 1.15]{Arfken2005}}
...
\end{align}
mit den zwei Eigenwerten $\lambda_{1,2}$ aus \eq{eq:A7BEE9E24E_lambdas},
...
\eq{sec:A7BEE9E24E_hyh} und \eq{sec:A7BEE9E24E_hyp} gleich sind!
%
Also: homogene DGL mit speziellen Anfangsbedingungen zu $t=0$ ist gleich wie
...
nennen diese Funktion auch Green'sche Funktion)
\begin{align}
h(t) =
\frac{1}{A}\,\frac{\e^{\lambda_1 t} - \e^{\lambda_2 t}}{\lambda_1 - \lambda_2}
\qquad \lambda_1 \neq \lambda_2.
...
also Störfunktion $x(t)$ angeben, und zwar mit dem Integral beginnend ab unserem gewähltem
Zeitpunkt $t=0$
\begin{equation}
...
Wir finden dieses Integral (mit allgemeineren Grenzen) in der Formelsammlung in dem
Abschnitt Faltung. Das Integral ist als \textbf{Faltungsintegral}, die Operation als
\textbf{Faltung} bekannt.
%
...
\begin{Werkzeug}
Entweder wir benutzen die Sachen aus dem vorangegangen Repetitorium
