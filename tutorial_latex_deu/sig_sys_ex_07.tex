...
\noindent Dirac Impulskamm
\begin{align}
x_\mathrm{s}(t) = x(t) \ast_t s(t) = \frac{1}{2\pi} \sum_{m=-\infty}^{+\infty} 2\mathrm{rect}(2[t-1\cdot m])
\end{align}
Rekonstruktion
\begin{align}
x_\mathrm{r}(t) = h_\mathrm{r}(t) \cdot x_\mathrm{s}(t) =
2\pi \mathrm{rect}(t) \cdot \left(
\frac{1}{2\pi} \sum_{m=-\infty}^{+\infty} 2\mathrm{rect}(2[t-1 \cdot m])
...
\end{align}
In der zweiten Formel-Zeile Dirac Multiplikationseigenschaft, für die fünfte Zeile
Dirac Austasteigenschaft, sinc ist die zeitgespiegelte Funktion im Faltungsintegral...
Anwendung bekannter Sachen.

Dieses Ergebnis kennen wir in allgemeiner Form aus \eq{eq:SamplingFreqSincInterp}.
%
Wir können mit Glg.~(1.12) aus Übung 1 leicht ermitteln, dass die Koeffizienten
...
(Zeitsignal ist eine periodische Rechteckfunktion) um damit
zur Fouriertransformierten (Zeitsignal ist ein einmaliger Rechteckimpuls) zur interpolieren.
