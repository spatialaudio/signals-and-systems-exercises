...
und Polstellen) zu übertragen; das Wesen bleibt ja gleich, 'nur' die Rechnerei wird u.U. komplizierter.
%
...
\end{itemize}
Das sind alles Dinge die in nachfolgenden Spezial-Fächern immer wieder benötigt
...
bereits bekannte Werkzeuge wieder antreffen; und erfreulich: das Wesen wird sich nicht ändern, es lohnt sich also das hier schon anzueignen!
...
Computer Algebra/Numerik zu schätzen wissen. Systeme höherer Ordnung, also noch mehr Null-
und Polstellen sind im Grund nicht mehr sinnvoll auf dem Papier zu handhaben (hat man vor ein paar Jahrzehnten noch in SigSys-Klausuren gebracht, für reines Rechenhandwerk sicher nice).
Daher werden wir in der Praxis diese Systeme aufsplitten in Reihen- oder
Parallelschaltung von Systemen 1. und 2. Ordnung. Es ist dann offensichtlich sehr sinnvoll,
...
Werkzeuge mit denen man bequem manuelle Rechnungen und Grafiken verifizieren kann.
...
einem Minimalphasensystem und einem Allpass.
\end{Ziel}
...
\draw[-] (-0.5-\tic,3) -- (-0.5+\tic,3) node[left]{$18$};
\draw[C0, -, ultra thick] (-0.5,3) -- (1,3) -- (3,1) -- (4,1);
\draw (3.5,+2.5) node[]{$\textcolor{C0}{20\mathrm{lg}|H(\omega)_\mathrm{min}|}$};
\draw (3.5,+2) node[]{$\textcolor{C0}{20\mathrm{lg}|H(\omega)_\mathrm{max}|}$};
\draw[C1, -, ultra thick] (-0.5,-3) -- (1,-3) -- (3,-1) -- (4,-1);
\draw (3.5,-2.5) node[]{$\textcolor{C1}{20\mathrm{lg}|H(\omega)_\mathrm{min}^{-1}|}$};
...
Faltung-Multiplikation-Dualität ab; also alles Dinge die wir schon gut kennen.
...
Das ist in der ET z.B. dann eine berechtigte Annahme, wenn wir Systeme
betrachten, die eher im Leerlauf (also weit weg vom Kurzschlußfall) betrieben werden.
...
genau die Prototypen von einzelnen Polen, Nullstellen in \fig{fig:magbodeprototype} (5.8).
...
\fig{fig:tikz_series_081294E23C}, \fig{fig:tikz_parallel_081294E23C}, \fig{fig:magbodeprototype} (5.8)
\end{Werkzeug}
%\begin{Ansatz}
...
\caption{für Aufgabe \ref{fig:frequency_group_8844932657}. \texttt{frequency\_group\_8844932657.ipynb}. Oben: Zeitsignal, unten: Betragsspektrum.}
...
%   \fourier
%   X(\omega) = \frac{1}{2\pi} \left([\pi\delta(\omega+\omega_0)+\pi\delta(\omega-\omega_0)]\ast\sqrt{\frac{\pi}{\alpha^2}}\e^{-\frac{\omega^2}{4\alpha^2}}\right)\\
%   &x(t) = \cos(\omega_0 [t-\tau]) \cdot \e^{-\alpha^2 [t-\tau]^2}
%   \fourier
%   X(\omega) = \frac{1}{2\pi} \left([\pi\delta(\omega+\omega_0)+\pi\delta(\omega-\omega_0)]\ast\sqrt{\frac{\pi}{\alpha^2}}\e^{-\frac{\omega^2}{4\alpha^2}}\right) \e^{-\im \omega \tau}
% \end{align}
% Mit Zahlen
% \begin{align}
%   &x(t) = \cos(\frac{1}{10} [t-600]) \cdot \e^{-(\nicefrac{3}{800})^2 [t-600]^2}
%   \fourier
%   X(\omega) = \frac{1}{2\pi} \left([\pi\delta(\omega+\frac{1}{10})+\pi\delta(\omega-\frac{1}{10})]\ast\sqrt{\frac{\pi}{(\nicefrac{3}{800})^2}}\e^{-\frac{\omega^2}{4(\nicefrac{3}{800})^2}}\right) \e^{-\im \omega 600}
% \end{align}
% vereinfacht
% \begin{align}
%   &x(t) = \cos(\frac{1}{10} [t-600]) \cdot \e^{-(\nicefrac{3}{800})^2 [t-600]^2}
%   \fourier
%   X(\omega) = \left([\delta(\omega+\frac{1}{10})+\delta(\omega-\frac{1}{10})]\ast\sqrt{\pi}\frac{400}{3}\e^{-\omega^2(\nicefrac{400}{3})^2}\right) \e^{-\im \omega 600}
% \end{align}
% Austast
% \begin{align}
%   &x(t) = \cos(\frac{1}{10} [t-600]) \cdot \e^{-(\nicefrac{3}{800})^2 [t-600]^2}
%   \fourier
%   X(\omega) = \sqrt{\pi}\frac{400}{3}\left(\e^{-[\omega+\frac{1}{10}]^2(\nicefrac{400}{3})^2}+\e^{-[\omega-\frac{1}{10}]^2(\nicefrac{400}{3})^2}\right) \e^{-\im \omega 600}
% \end{align}
% \begin{align}
%   &x(t) = \cos\left(\frac{1}{10} [t-600]\right) \cdot \e^{-\left(\nicefrac{3}{800}\right)^2 [t-600]^2}\\
%   &\fourier\\
