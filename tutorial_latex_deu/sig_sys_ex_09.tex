...
%
Die $z$-Transformation bildet zeitdiskrete Signale $x[k]$
($x\in\mathbb{C}$, $k\in\mathbb{Z}$)
auf Funktionen $X(z)$ ($X\in\mathbb{C}$, $z\in\mathbb{C}$)
ab.
%
Wir arbeiten im Bildbereich also in der komplexwertigen $z$-Ebene mit
komplexwertigen Funktionen $X(z)$.
%
In speziellen aber in SigSys oft benutzen Fällen ist $x\in\mathbb{R}$, also
reelle Folgenglieder bzw. Samples.
%
Wie üblich versuchen wir so viel wie möglich mit einfachen Rechnungen,
aber viel Anschauung zugänglich zu machen.
...
\end{align}
%
Die Rücktransformation ist im Grunde 'nur' die Anwendung des Cauchy
Integralsatzes, wie sich mit ein paar Zeilen Rechnung zeigen lässt, vgl.
...
\text{Exponenten zusammen:   } X(z) z^{k-1} =& \sum_{k'=-\infty}^{\infty} \frac{x[k']}{z^{k'-k+1}}\\
...
\end{itemize}
Die Summe über alle $k'$ liefert also die obige Rücktransformationsformel
\begin{align}
...
z.B. $X(z), H(z), Y(z)$ und Reihenkoeffizienten $c_k$ sind bei uns eher
zeitdiskrete Signale $x[k], h[k], y[k]$.
%
Schreiben wir es also für die Übersichtlichkeit nochmal untereinander
\begin{align}
\text{SigSys z-Trafo: } X(z) = \sum_{k=-\infty}^{\infty} x[k] \, z^{-k}\qquad
&x[k] = \frac{1}{2\pi \im} \oint\limits_{C \subset \text{KB}} X(z) \, z^{k-1} \, \fsd z\\
\text{Mathe Laurent Reihe:  } f(z) = \sum_{k=-\infty}^{\infty} c_k (z - z_0)^k\qquad
&c_k = \frac{1}{2\pi\im}\oint\limits_{C \subset \text{KB}}
f(z) (z-z_0)^{-k-1}\fsd z.
\end{align}
...
konsistent sind.
%
Die SigSys Schreibweise ist ein wenig eleganter für SigSys, weil Ergebnisse
...
erfolgen.
%
Wir werden das Integral und den Residuensatz im Rahmen dieser Übung (leider
aus Zeitmangel) nie benutzen, sondern uns wieder mit bekannten Korrespondenzen,
...
Normierung
\begin{align}
1 \cdot y[k] = \sum_{m=0}^M (\frac{b_m}{a_0}) x[k-m] - \sum_{n=1}^N (\frac{a_n}{a_0}) y[k-n].
\end{align}
%
Eine der fundamentalen Eigenschaften, nämlich Zeitverschiebung führt zu
Phasenverschiebung im Spektrum (das ist der Verschiebungssatz)
\begin{align}
x[k-\kappa] \quad\ztransf\quad z^{-\kappa} \cdot X(z),
\end{align}
d.h. z.B. die Verzögerung des Dirac Impulses um \textbf{einen} Folgenindex (Abtastwert)
führt zu
\begin{align}
...
der KB im inneren eines Kreises zu finden ist, und für \textbf{beidseitge Signale}
...
\textbf{Eigenfolgen von zeitdiskreten Systemen sind}, und das interessiert uns
in der SigSys speziell.
%
Der einfachste Fall ist $a=1$, dann bekommen wir den Einheitssprung und dessen
Korrespondenz.
%
...
Aus \fig{fig:single_pole_rightsided_tikz} oben ist uns die Rücktransformation
...
\end{align}
Dies setzen wir in \eq{eq:Y_HX_zTrafo_A1D74A9E5B} ein. Wir erhalten
\begin{align}
...
zu mischen, also z.B. $z^{-1}$ im Blockschaltdiagramm des Zeitbereichs zu verwenden,
obwohl es da streng genommen nichts zu suchen hat. Das Diagramm sieht dann wenig
übersichtlicher aus, daher wir die diese Konvention gerne gewählt, also:
\begin{center}
\begin{tikzpicture}[thick, scale=0.7]
\newcommand{\filtDelay}{$z^{-1}$}
%\newcommand{\filtDelay}{$\ast_k \delta[k-1]$}
...
und wir haben hier ein konjugiert-komplexes Polpaar, daher benötigen wir den
Ansatz
%
\begin{align}
H(z) =& \frac{z^2 - z + 2}{z^2 - \frac{1}{2}z + \frac{1}{4}}\\
...
\end{align}
$1/z$ von links wieder nach rechts bringen
\begin{align}
...
r^k \sin(\Omega_0 k)\epsilon[k] \ztransf \frac{(r\sin\Omega_0)z}{z^2 -(2r\cos\Omega_0)z + r^2}
\end{align}
zerlegen können. Das war SigSys-Wissen, jetzt wieder stumpf ausrechnen: Fangen
...
in Systeme 2. Ordnung (englisch sogenannte second order sections, SOSs),
...
%
\texttt{/z\_system\_analysis/z\_system\_analysis.ipynb}
}
