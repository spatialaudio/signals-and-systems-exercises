...
%
\begin{Ziel}
Wir wollen hier an einem sehr einfachen Beispiel (das sind die besten!) erarbeiten,
...
\textbf{a) Blockschaltbild}
%
\begin{center}
\begin{tikzpicture}[scale=1]
\newcommand{\filtDelay}{$z^{-2}$}
\tikzstyle{filtBlock} = [draw, rectangle, minimum height=2em, minimum width=2em,anchor=center]
...
\node(input)[anchor=center]{$x[k]$};
& \node (ffsplit0)[filtBranch]{}; & \node (join0)[filtSum]{$+$}; & \node (output)[anchor=center]{$y[k]$}; \\
& \node (delay0)[filtBlock] {\small \filtDelay}; &\node (b1) [filtBlock]{\BEins}; \\
};
\draw [->] (input) -- (ffsplit0) -- (join0);
\draw [->] (join0)-- (output);
\draw [->] (ffsplit0)-- (delay0);
\draw [->] (delay0)-- (b1);
\draw [->] (b1) -- (join0);
\end{scope}
\end{tikzpicture}
...
\textbf{b) Übertragungsfunktion}
\begin{align}
&1 \cdot y[k] = 1 \cdot x[k] - 1 \cdot x[k-2] \quad\ztransf\quad 1 \cdot Y(z) = 1 \cdot X(z) - 1 \cdot X(z) \cdot z^{-2}\\
&H(z) = \frac{Y(z)}{X(z)} = \frac{1-z^{-2}}{1} = \frac{z^2-1}{z^2} = 1 \cdot \frac{(z-1)(z+1)}{(z-0)(z-0)}
\end{align}

\textbf{c) Pole, Nullstellen, Verstärkungsfaktor, Konvergenzbereich, PN-Diagramm}

nur Pole im Ursprung, d.h. Konvergenz für alle $z$, außer $z=0$, d.h. im PN-Diagramm, alles grün, außer der Ursprung!
...
\begin{tikzpicture}[scale=1.5]
\def \tic {0.4}
\filldraw[even odd rule,C2!50] (0,0) circle(0.025)
decorate [decoration={snake,segment length=15pt,amplitude=1pt}]{(0,-3pt) circle(1.6)};
\draw[help lines, C7!50, step=0.25cm] (-1.5,-1.5) grid (1.5,1.5);
\draw (0,0) circle(1);
...
\node at (1, +1){$H_0=1$};
\node at (1, -1){$\text{KB: } |z|>0$};
\draw[->] (-1.8,0)--(1.9,0) node[right]{$\Re\{z\}$};
\draw[->] (0,-1.8)--(0,1.8) node[above]{$\Im\{z\}$};
\node[C0] at (0,0){\Huge $\times$};
\node[C0] at (0.175,-0.2){$(2)$};
\node[C0] at (+1,0){\Huge $\circ$};
\node[C0] at (-1,0){\Huge $\circ$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1]
\def\tic{0.1};
\draw[->] (-2.5,0) -- (3.5,0) node[right]{$k$};
\draw[->] (0,-1.5) -- (0,1.5) node[above]{$h[k]$};
\foreach \x in {-2,...,2}{\draw (\x,-\tic) -- (\x,+\tic)  node[above]{$\x$};};
\foreach \y in {-1,...,1}{\draw (\tic,\y) -- (-\tic,\y)  node[left]{$\y$};};
\draw[stem2, C0] plot coordinates{(-2,0) (-1,0) (0,1) (1,0) (2,-1) (3,0)};
\end{tikzpicture}
\end{center}
%
...
$x[k]\rightarrow \delta[k]$ in die DGL einsetzen, um $h[k]$ zu erhalten
\begin{align}
y[k] = x[k] - x[k-2]
\quad\rightarrow\quad
h[k] = \delta[k] - \delta[k-2]
\end{align}
Gleiches Ergebnis, gleiches endliches Signal. Wenn wir diese Impulsantwort als
...
\def\tic{0.1};
\draw[->] (-2.5,0) -- (3.5,0) node[right]{$k$};
\draw[->] (0,-1.5) -- (0,1.5) node[above]{$h_\epsilon[k]$};
\foreach \x in {-2,...,2}{\draw (\x,-\tic) -- (\x,+\tic)  node[above]{$\x$};};
\foreach \y in {-1,...,1}{\draw (\tic,\y) -- (-\tic,\y)  node[left]{$\y$};};
\draw[stem2, C0] plot coordinates{(-2,0) (-1,0) (0,1) (1,1) (2,0) (3,0)};
\end{tikzpicture}
\end{center}
...
%
Die Sprungantwort eines Systems mit endlicher Impulsantwort ist auch immer endlich.
