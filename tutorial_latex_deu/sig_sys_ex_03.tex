...
\cleardoublepage
\subsection{Von der Essenz zur Anwendung: RC-Glied / Tiefpass 1. Ordnung / PT1-Glied}
...
\end{bmatrix}
\end{align}
% wolfram alpha: inv([[1,1,0], [0,t,1], [t^2,0,t]]) * [[0],[1],[0]]
% \begin{align}
% A =& -B\\
...
xlabel = {$t\rightarrow$},
ylabel = {$h_\epsilon(t)$},
title = {Step Response / Sprungantwort},
xmin=-0.1, xmax=7,
ymin=-0.1, ymax=1.1,
xtick={0, 1, 3, 5},
ytick={0,0.63,0.95, 1},
...
\cleardoublepage
\subsection{Problematik Anfangsbedingungen beim CR-Glied / Hochpass 1. Ordnung / DT1-Glied}
...
\begin{align}
%s \cdot X(s) - x(t=0^-) = s \cdot Y(s) - y(t=0^-) + \frac{Y(s)}{T_\text{RC}}\\
%s \cdot X(s) - x(t=0^-) + y(t=0^-) = (s + \frac{1}{T_\text{RC}}) \cdot Y(s)\\
%s \cdot X(s) + y(t=0^-) - x(t=0^-) = (s + \frac{1}{T_\text{RC}}) \cdot Y(s)\\
...
\draw (-\sigmaz,\tic)--(-\sigmaz,-\tic) node[below]{$-\frac{1}{T_\mathrm{RC}}$};
\draw (1.75,+2.25) node[C2!75]{KB: $\Re\{s\}>-\frac{1}{T_\mathrm{RC}}$};
\draw (1.25,1.75) node[]{$g=1$};
\draw (1.25,-1.5) node[]{PNV für $H(s)$};
\end{tikzpicture}
\end{center}
\end{figure}
...
$H(s)$ ist keine echte gebrochen-rationale Funktion, weil Polynomgrad Zähler = Nenner. Erst eine Polynomdivision \, \polylongdiv[style=C, vars=s]{s}{s+w} \quad führt auf eine Parallelschaltung zweier Systeme
\begin{align}
...
ylabel = {$y(t)$},
title = {Anfangsbedingungen \& Eingang},
ylabel = {$\textcolor{C2}{x(t=0^-)},\,\,\,\textcolor{C4}{y(t=0^-)},\,\,\,\textcolor{C5}{x(t \geq 0)}$},
xmin=-5, xmax=20,
ymin=-2, ymax=3,
ytick={-2, -1, 0, 1, 2},
ymajorgrids=true,
...
\newpage
\subsection{Laplace für Hochpass 2. Ordnung}
...
\draw[C0, ultra thick] (-\sigmaz,-\omegaz) node{\Huge $\times$};
\draw[C0, ultra thick] (0,0) node{\Huge $\circ$};
\draw[C0, ultra thick] (0,0) node{\Large $\circ$};
\draw (-\sigmaz,\tic)--(-\sigmaz,-\tic) node[below]{$-\frac{2}{5}$};
\draw (-\tic,\omegaz) -- (\tic,\omegaz) node[right]{$+\frac{1}{6}$};
\draw (-\tic,-\omegaz) -- (\tic,-\omegaz) node[right]{$-\frac{1}{6}$};
\draw (1.5,+2.25) node[C2!75]{KB $\Re\{s\}>-\frac{2}{5}$};
\draw (1.25,1.75) node[]{$g=1$};
\end{tikzpicture}
\end{center}
\end{figure}
...
\, \polylongdiv[style=C, vars=s]{s^2}{s^2+4/5*s+169/900} \quad können wir eine Parallelschaltung von Systemen erzeugen; der entstehende Bruch ist nun eine echt gebrochen-rationale Funktion, weil Zählergrad < Nennergrad.
%
