...
aufschreiben, das wird so sehr gerne in Physik und Mathe benutzt
\begin{align}
...
\qquad
\bm{x}_\mu = \left(\mathring{\bm{F}}\right)^* \, \bm{x}_k,
\end{align}
...
DFT-Spektrum) die Koeffizienten einer Linearkombination
aller Vektoren der (fundamental wichtigen) orthonormalen Fourier-Vektorbasis repräsentieren.
...
verstanden haben, können wir die Spektren beliebig verkomplizieren und damit alle mögliche
$N$ periodische Signalfolgen synthetisieren.
Die Vorgehens- und Sichtweise ist sehr ähnlich zur Signalsynthese mit der komplexen
Fourierreihe.
\end{Ziel}
...
immer wieder neu über numerische Ergebnisse zu wundern ;-) Wir werden
...
(der Erkenntnisgewinn hält sich in Grenzen, daher können wir hier mal guten Gewissens auf Mathe mit Papier \& Stift verzichten
, außer wir wollen ganz viel Euleridentität üben).
%
...
intensiver anschauen...Und erst dann mit einem Computer rumspielen, aber vielleicht nicht unbedingt ChatGPT danach fragen, das Tool hat hier in dem ganzen SigSys-Kontext nichts zu suchen! Wolfram Alpha schon eher...aber keine Ahnung wie man das da macht.
...
%
Schuld ist \textbf{nie} die DFT, die macht alles richtig, immer!!. Die DFT bildet
'einfach nur' einen $N$-dimensionalen Vektor auf einen anderen ab. Linearkombination und Lineare Algebra anzweifeln ist daher kein Mittel der Wahl ;-)
%
...
nicht erlaubt ist, aber dadurch wird es besser ersichtlich) sehen wir das Signal
\begin{align}
...
Die Fouriermatrix kann absolut nichts für unser Unvermögen einen nicht repräsentativen
Signalausschnitt ausgewählt zu haben (also wieder bildlich: einen Sinus hart abgeschnitten zu haben, den wir eigentlich gerne in schön analysieren wollten).
%
...
(diese sind komplett definiert durch die erste Spalte oder die erste Zeile, der Rest kann nach einem Algorithmus aufgefüllt werden).
...
oder die erste Zeile ausrechnen müssen, um die gesamte Matrix zu kennen.
%
