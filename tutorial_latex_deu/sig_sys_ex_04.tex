...
welche Trafo benutzt wird. In der Formelsammlung und Übung 1 ist das z.B. eindeutig, weil
wir dort keine Laplace Transformation verwenden.
%
...
\frac{1}{|a|} X(\frac{\im\omega}{a}) \e^{-\im\omega \frac{b}{a}}
\end{align}
...
\draw[->] (0,-1.1) -- (0,1.25) node[above] {$X_r(\im\omega)=\textcolor{C1}{\pi\delta(\omega+\omega_0)}+\textcolor{C0}{\pi\delta(\omega-\omega_0)}$};
...
\begin{scope}[shift={(6,0)}]
\draw[->] (-2.1,0) -- (2.1,0) node[right] {$\omega$};
\draw[->] (0,-1.1) -- (0,1.25) node[above] {$X_i(\im\omega)=\textcolor{C1}{\im\pi\delta(\omega+\omega_0)}-\textcolor{C0}{\im\pi\delta(\omega-\omega_0)}$};
...
$x_r(t)=1 \, \fourier \, X_r(\im\omega)=2\pi\delta(\omega)$ und
$x_i(t)=0 \, \fourier \, X_i(\im\omega)=0$ gelten.}
...
\begin{scope}[shift={(6,0)}]
\draw[->] (-2.1,0) -- (2.1,0) node[right] {$\omega$};
\draw[->] (0,-1.1) -- (0,1.25) node[above] {$X(\im\omega)=\textcolor{C0}{2\pi\delta(\omega-\omega_0)}$};
...
der un-intuitivste; und ohne Erwartungshaltung Rechnen ist auch nicht die beste
...
\end{Ansatz}
\begin{ExCalc}
Die Korrespondenz für den Sinus ist (siehe \fig{fig:ue4_cos_sin_ft})
\begin{align}
...
\draw[->] (0,-1.1) -- (0,1.25) node[above] {$X(\im\omega)=\textcolor{C0}{\frac{-\pi}{2}\delta(\omega+2\omega_0)}+\textcolor{C2}{\pi\delta(\omega)}+\textcolor{C3}{\frac{-\pi}{2}\delta(\omega-2\omega_0)}$};
...
(Konvergenzbereich muss $\Im\{s\}$ enthalten),
...
Hier also wegen gewünschter Didaktik, der Bildbereich. Für die Energiebetrachtung spielt die
Phase keine Rolle.
\begin{align}
