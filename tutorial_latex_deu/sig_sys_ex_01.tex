...
\end{cases}
\end{align}
als Eigenschaft haben soll. Mit der zweiten Formel kann man schlecht rechnen, daher Distribution, Grenzübergang, Definitionsintegral usw., siehe oben.
...
\text{Austasteigenschaft \circled{1} allgemein} & \quad \int\limits_{t=-\infty}^{+\infty} \delta(t-\tau) x(t)\fsd t = x(\tau)\\
\text{Austasteigenschaft \circled{1} für } \tau=0& \quad \int\limits_{t=-\infty}^{+\infty} \delta(t) x(t)\fsd t = x(0)
\end{align}
%
Diese Austasteigenschaft können wir mit einem Variablentausch $t \leftrightarrow \tau$ als
\begin{align}
\int\limits_{\tau=-\infty}^{+\infty} \delta(\tau-t) x(\tau)\fsd \tau = x(t)
\end{align}
schreiben. Der Dirac Impuls ist ein gerades Signal, d.h. es gilt $\delta(t)=\delta(-t)$. Es gilt also auch $\delta(\tau-t)=\delta(-(\tau-t))=\delta(-\tau+t)$ und man kann damit die
%Austasteigenschaft \circled{2}  allgemein
\begin{align}
\text{Austasteigenschaft \circled{2} allgemein} & \quad \int\limits_{\tau=-\infty}^{+\infty} \delta(-\tau+t) x(\tau)\fsd \tau = x(t),
\end{align}
aufschreiben, die sich auch in der Formelsammlung findet. Typ \circled{1} und \circled{2} sind nützlich für verschiedene Rechenwege. Typ \circled{1} ist in den meisten Lehrbüchern drin und mutmaßlich am Anfang einfacher zu verdauen. Typ \circled{2} ist sehr gut geeignet, die Faltung elegant herzuleiten; und weil Faltung so fundamental wichtig ist in SigSys, steht \circled{2} in der Formelsammlung.
