...
%
%
In the previous section we calculated the inverse Laplace transform with using partial fraction decomposition and correspondence tables. Now we want to find the solution using the complex integral
Before we are starting to calculate the inverse Lapplace transform, we have to take a step back to math lecutre, complex analysis. First, there are some definitions.\\
\subsubsection{Isolated singularity} 


May $U \subset \mathbb{C}$ be open, $z_0 \in U$ and $f : U \setminus \{z_0\} \rightarrow \mathbb{C}$ holomorph. Then $z_0$ is named \textbf{isolated singularity} of $f$ [\cite{Fritzsche2019}].\\

Types of isolated singularities (can be found in [\cite{Fritzsche2019}]): \\

May $U \subset \mathbb{C}$ open and $f$ holomorph on $U$ until a isolated singularity in point $z_0 \in U$.
\begin{enumerate}
	\item $z_0$ is a \textbf{removeable singularity} of $f$, if there is a holomorph function $g$ on $U$, so that $f(z)=g(z)$ for $z \in U \subset \{z_0\}$.
	...
	\item $z_0$ is \textbf{essential singularity}, if it is neither a removeable singularity nor a pol.
\end{enumerate}
\subsubsection{Laurent series} 
A Laurent series is a series with the form:
\begin{align}
	f(z) = \sum_{k=-\infty}^{+\infty}c_k\cdot(z-z_0)^k.
\end{align}
You can find it in [\cite{Fritzsche2019}]. \\
\subsubsection{Residue}
The following definition and the connection with the Laurent series can be found in [\cite{Fritzsche2019}]. \\
...
The coefficent $c_{-1}$ of the Laurent series of $f$ around $z_0$ is the residue of $f$ in $z_0$:
\begin{align}
	\mathrm{Res}(f(z),z_0)=c_{-1}.
\end{align}
\subsubsection{Calculation of residue}
If $z_0$ is a removeable singularity:
\begin{align}
	\mathrm{Res}(f(z),z_0)=0,
\end{align}
because $c_k$ = 0 for $k<0$ [\cite{Fritzsche2019}]. \\
Example:
\begin{align}
	f(z) &= \frac{\sin(z)}{z} \nonumber \\
	\lim\limits_{z \rightarrow 0}f(z) &= \lim\limits_{z \rightarrow 0}\frac{\cos(z)}{1}=1 \Rightarrow \mathrm{Res}(f(z),0)=0 \nonumber \\
	\sin(z) &= z-\frac{z^3}{3!}+\frac{z^5}{5!}-+\cdot\cdot\cdot\quad \quad \textrm{[\cite{Bronstein}]} \nonumber \\
...
\end{align}
If $z_0$ is essential singularity, you have to find the residue by developing the laurent series.
\begin{align}
	f(z)=\e^{\frac{1}{z}} \nonumber 
\end{align}
\begin{align}
	\e^z=1+z+\frac{z^2}{2!}+\frac{z^3}{3!}+\frac{z^4}{4!}+\cdot\cdot\cdot\quad\quad \textrm{[\cite{Bronstein}]} \nonumber \\
\end{align}
Now substituting $z$ with $\frac{1}{z}$ results in
\begin{align}
	f(z)=z^{-1}+1+\frac{z^1}{2!}+\frac{z^2}{3!}+\frac{z^3}{4!}+\cdot\cdot\cdot .
\end{align}
\begin{align}
	\rightarrow\mathrm{Res}(f(z),0)=1
\end{align}
\subsubsection{Residue theorem} 
You can find the following definition in [\cite{Fritzsche2019}]. \\
May $G \subset \mathbb{C}$ an connected space, $D \subset G$ finite, $\gamma$ a closed curve in $G$ with $|\gamma| \cap D = \emptyset$ and $f: G \setminus D \rightarrow \mathbb{C}$ holomorph. Then is 
\begin{align}
	\frac{1}{2\pi\im}\int_{\gamma}f(\xi)\mathrm{d}\xi = \sum_{z\in D}n(\gamma,z)\mathrm{Res}(f(z),z).
\end{align}
That means, we can calculate complex circle integrals by adding $\mathrm{Res}(f(z),z_1$) to $\mathrm{Res}(f(z),z_n)$ for all sourrounded isolated singularities $z_1$ to $z_n$.
...
\end{align}
[\cite{UlrichWeber2017}]. 
We chose a curce $C = C_1 + C_2$, where $C_1$ is a curve parallel to the imaginary axes through $\sigma$ from $\sigma -\im\omega_0$ to $\sigma + \im\omega_0$ and $C_2$ a part of a circle with center at $0$ from $\sigma +\im\omega_0$ to $\sigma - \im\omega_0$ [\cite{UlrichWeber2017}].
Based on figure 3.5 in [\cite{UlrichWeber2017}], here is the curve in parameter form:
\begin{align}
...
\end{align}
[\cite{UlrichWeber2017}]. 
If $\omega_0$ is chosen big enough, the complex integral $\oint_CY(s)\e^{st}\mathrm{d}s$ sourounds all isolated singularities $z_1$ to $z_n$ of $Y(s)$ and we can use the residue theorem:
\begin{align}
...
\end{align}
[\cite{UlrichWeber2017}]. If we let $\omega_0\rightarrow\infty$, then also $R\rightarrow\infty$ and it can be shown, that $B \rightarrow 0$ for $R\rightarrow \infty$ [\cite{UlrichWeber2017}]. In the end we get
\begin{align}
...
\begin{align}
	F(s)=\frac{1}{s^2}.
\end{align}
$0$ is pole with order 2 and the only singularity of $F(s)$.
\begin{align}
	\mathrm{Res}(F(s)\e^{st},0)=\lim\limits_{s\rightarrow0}\frac{1}{(2-1)!}\cdot\frac{\mathrm{d}}{\mathrm{d}s}\frac{\e^{st}\cdot s^2}{s^2}=\lim\limits_{s\rightarrow0}t\e^{st}=t
\end{align}
We achieve
\begin{align}
	y(t)=\mathcal{L}^{-s}[Y(s)]=t.
\end{align}
...
\subsubsection{Task b) with Convolution: Particular solution with x(t) = Unit Step}
...
\subsubsection{Task e) with Convolution: Particular solution with x(t>0) = sin(t)}
%
